...
def find_bg(DATA, site, atol=0.01, energy_window=1.0, kidx=None):
	"""Find bandgap from LDOS data at given site index.
	The `DATA` dictionary is of the computations for a given structure.  
	The `site` is the index of the atom in the device to consider for LDOS.  
	`atol` is the threshold below which LDOS is considered zero (i.e., in bandgap).  
	`energy_window` is the energy range around Fermi level (0 eV) to consider for bandgap search.  
	`kidx` is the k-point index to consider (if multiple k-points were used in calculation).
	"""
	data = dict(DATA)
